...
Gitリポジトリを操作し、カットのバージョン管理・承認フローを実現するクラス。すべての書き込み操作は `asyncio.Lock` で保護されており、同時実行時も安全。

#### 関連 Enum / データクラス

##### `GitCommands` (StrEnum)

git コマンド文字列の定数。値: `SHOW`, `COMMIT`, `CHECKOUT`, `MERGE`, `PUSH`, `ADD`, `INIT`, `LOG`, `STATUS`

##### `SA_CommitType` (StrEnum)

コミットメッセージに埋め込むアクションの種別。
...
| 引数 | 型 | 説明 |
|---|---|---|
| `git_repo_local_dir` | `str \| None` | Gitリポジトリのパス。`None` の場合は `project_settings.json` の `git_repo_local` を使用 |

---

##### `get_components(cut_num) -> list[str]`

`project_main.json` から指定カットのコンポーネント名リストを取得する。
...
Gitリポジトリを初期化し、`main` と `pending` の2ブランチを作成する。新規プロジェクト作成時に1度だけ呼び出す。

| 引数 | 型 | 説明 |
|---|---|---|
| `proj_settings` | `dict` | プロジェクト設定辞書。`"cut_num"` (int) と `"components"` (dict) を含む必要がある |
...
Gitリポジトリから指定ブランチ・カット・コンポーネントの最新情報を取得する。`repointer` が設定されている場合は自動的に参照先カットの情報を返す。

| 引数 | 型 | 説明 |
|---|---|---|
| `branch` | `ShellArcGitBranch \| str` | 取得元ブランチ（`"pending"` or `"main"`）|
| `cut_num` | `int` | カット番号 |
| `component` | `str` | コンポーネント名 |
| `commit_id` | `str \| None` | 特定のコミットIDを指定する場合。`None` はブランチの最新（デフォルト: `None`）|

**返り値** : `dict[str, str]` — コンポーネントJSONの内容。データが存在しない場合は空の辞書 `{}`。
...
| 引数 | 型 | 説明 |
|---|---|---|
| `output_format` | `list[int]` | コミットレコードのどのフィールドを返すかを示すインデックスリスト |
...
**コミットレコードのフィールドインデックス：**

| インデックス | フィールド名 | 説明 |
|---|---|---|
| 0 | `commit_type` | コミットタイプ（`SUBMIT`, `APPROVE` 等）|
| 1 | `cut_num` | カット番号（文字列）|
| 2 | `component` | コンポーネント名 |
| 3 | `creator_name` | 作成者名 |
| 4 | `commit_message` | 任意メッセージ |
| 5 | `timemark` | タイムスタンプ（JST, `YYYYMMDDHHMMSS`）|
| 6 | `file_index_name` | ファイルインデックス名（SUBMIT時のみ）|

**返り値** : `dict[str, str]` — `{コミットハッシュ(short): "フィールド1 フィールド2 ..."}` の形式。フィールドはスペース区切り。

**使用例：**

```python
# SUBMITコミットのタイムスタンプとfile_index_nameを取得
log_filter = SA_GitLogFilter(commit_type=SA_CommitType.SUBMIT, log_length=5)
hist = await git_io.get_log(
    output_format=[5, 6],           # timemark と file_index_name
    log_filter=log_filter,
    limit_scope="stage/cut1/modeling.json"
)
# → {"abc1234": "20240101120000 cut1_modeling_abc123_20240101120000", ...}
```
...
コンポーネントの新規提出（SUBMIT）を行う。pendingブランチにコミットを作成し、`.sa_pending_{component}` ファイルを生成する。
...
**返り値** : `str` — 生成されたファイルインデックス名。R2へのアップロードパス生成に使用する。形式: `cut{N}_{component}_{creator_id_6char}_{timemark}`。`*` を含む文字列は `+` に置換される。

---

##### `async pend_data(cut_num, component, processing_person, is_approve, message="") -> None`

レビュー担当者による承認または却下を行う。

**承認（`is_approve=True`）の場合：**
1. `.sa_pending_{component}` を削除
2. `pending` ブランチにAPPROVEコミット
3. `pending` → `main` ブランチへコンポーネントJSONをチェックアウト
4. `main` ブランチにAPPROVEコミット

**却下（`is_approve=False`）の場合：**
1. `.sa_pending_{component}` を削除
2. `pending` ブランチにDECLINEコミット（mainへの反映なし）
...
**送出例外：**
- `SA_InvalidRequestObj` : `.sa_pending_{component}` が存在しない（ペンディングデータがない）
- `SA_LocalIOError(SA_8002)` : Gitコマンドエラー（失敗時は `.sa_pending_{component}` をロールバックして再生成）

---

##### `async repoint_data(be_repointed_cut, repoint_target_cut, component) -> None`

カットのコンポーネントデータを別カットのデータへ参照付け替えする。例えばカット5がカット3と同じ素材を使う場合に使用。
...
あるカットのコンポーネントJSONを別カットにコピーして取り込む。`repoint_data` とは異なり、参照ではなくデータのコピーを行う。

| 引数 | 型 | 説明 |
|---|---|---|
| `absorbing_cut` | `int` | 取り込む側のカット番号 |
| `absorb_target_cut` | `int` | 取り込まれる側のカット番号 |
| `component` | `str` | コンポーネント名 |
| `commit_id` | `str \| None` | 特定コミットIDのデータを取り込む場合。`None` はブランチの最新 |
| `branch` | `ShellArcGitBranch` | 取り込み元のブランチ（デフォルト: `PENDING`）|
