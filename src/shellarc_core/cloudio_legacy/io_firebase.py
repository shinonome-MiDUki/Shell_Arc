...
from shellarc_core.auth.access_database import AccessDB as A_DB
...
class DB_IO:
    def __init__(self,
        ...
        a_db = A_DB()
        self.db = a_db.database
        cfg_io = Cfg_IO()
        collection_name = cfg_io.get_cfg_setting(Cfg_item.COLL_NAME)
        ref_collection = self.db.collection(collection_name).document("main_proj")
        self.ref_requested_data = ref_collection.collection(f"cut{cut_num}").document(processing_component).get()
        ref_requested_data_c = self.ref_requested_data
        self.requested_data = ref_requested_data_c.to_dict()
    ...
    def get_info(self,
        ...
        requested_info = str(requested_status_dict[request_info.value])
    ...
    def update_remote(self,
        ...
        try:
            self.ref_requested_data.update(structure)
        except Exception as e:
            raise SA_CommunicationError(
                error_log=f"Error when uploading data to remote db [{e}]",
