# Shell Arc - プロジェクト管理システム

# 機能
- **プロジェクトデータシステム自動構築** - 設定ファイルのテンプレートを編集するだけで、Firestoreデータベース、Cloudflare R2ストレージとGoogleスプレッドシートが自動的に整備されます
- **ファイル提出・取得・レビュー一元管理** - StreamlitベースのUIを通じて、ファイルの自動再命名から提出、取得、レビューまで簡単に行えます
- **Discordボット連携** - Discordを通じて、プロジェクトのデータシステムへのファイル提出が可能になります
- **無限のカスタマイズ性** - オープンソース設計と柔軟のコードベースにより、プロジェクトの特定ニーズに合わせたカスタマイズしたり、他のサービスに連携したりすることが容易にできます
