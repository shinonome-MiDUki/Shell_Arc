...
from test_shellarc_core.exception.user_exception import SA_DataNotExist, SA_InvalidUserQuery
from test_shellarc_core.exception.structure_error import SA_LocalIOError, SA_ErrorCode
...
class ShellArc_Request:
    ...
    def download_material(self,
        ...
        check_pt_2 = self.r2_io.download_file(
            to_download_file=to_download_file,
            download_destination=temp_dir,
            file_naming=name_with_ext
        )
        check_pt_3 = to_download_file
        full_temp_path = os.path.join(temp_dir)
        ...
        os.rmdir(temp_dir)
        return {"cp1" : check_pt_1, "cp2" : check_pt_2, "cp3" : check_pt_3}